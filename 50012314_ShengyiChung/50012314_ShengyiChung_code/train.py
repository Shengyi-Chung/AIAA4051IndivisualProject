...
EXPERIMENT_TAG = "lr1e4-r32-e3-d01-bs4-ga4_maskq-false-padright"
DEFAULTS = {
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=DEFAULTS["compute_train_accuracy"],
        nargs='?',
        const=True,
        metavar='BOOL',
        help="Whether to run generation-based accuracy on train split at the end",
    )
...
def main():
    ...
    val_accuracy, eval_logs = run_accuracy_eval(model, tokenizer, val_data)

    train_accuracy = None
    if args.compute_train_accuracy:
        print("Evaluating on train set...")
        train_accuracy, _ = run_accuracy_eval(model, tokenizer, train_data)
    ...
    if use_wandb:
        try:
            ...
            wandb.log({
                ...
                "eval/val_accuracy": val_accuracy,
                ...
                "hparams/compute_train_accuracy": int(args.compute_train_accuracy),
            })
            if train_accuracy is not None:
                wandb.log({"eval/train_accuracy": train_accuracy})
            wandb.finish()
        except Exception as e:
